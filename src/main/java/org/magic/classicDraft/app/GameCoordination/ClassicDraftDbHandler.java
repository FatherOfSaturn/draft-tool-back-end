...
public class ClassicDraftDbHandler {
    ...
    public Document findDraftData(String gameID, String playerName) {
        Bson filter = Filters.eq("gameID", gameID);

        Bson addFieldsStage = Aggregates.addFields(
            new Field<>("player", new Document(
                "$arrayElemAt", java.util.List.of(
                    new Document("$filter", new Document()
                        .append("input", "$players")
                        .append("as", "p")
        ...
        Bson projectStage = Aggregates.project(Projections.fields(
            Projections.excludeId(),
            Projections.include("gameID", "gameState", "draftDirection"),
            Projections.include("player")
        ));
