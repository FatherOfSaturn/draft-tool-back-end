...
class ClassicGameCoordinationWorkerTest {
    ...
    void testStartGame() throws IOException {
        ClassicGameCreationInfo info = new ClassicGameCreationInfo("game-1", "cube-1",
                List.of(
    ...
    void testGetDraftData() {
        ...
        when(dbHandler.findDraftData("game-1", "Alice"))
                .thenReturn(new org.bson.Document("gameID", "game-1")
                        .append("gameState", "GAME_STARTED")
                        .append("draftDirection", "ASCENDING"));
        when(dbHandler.findGame("game-1")).thenReturn(game);
        ...
        assertEquals("Alice", result.player().getPlayerName());
    }
